    ...
    <div class="bg-white shadow rounded-lg overflow-hidden max-w-3xl mx-auto">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6 space-y-6">
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm" required>
                            @error('email')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="password" name="password" id="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm">
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" id="password_confirmation" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm">
                        </div>
                        
                        <!-- Phone Number -->
                        <div>
                            <label for="phone_number" class="block text-sm font-medium text-gray-700">Phone Number</label>
                            <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $user->phone_number) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm">
                            @error('phone_number')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="mt-1 flex items-center">
                                @if($user->profile_image)
                                    <img src="{{ Storage::url($user->profile_image) }}" alt="{{ $user->name }}" class="h-12 w-12 object-cover rounded-full mr-3">
                                @else
                ...
                <div id="organization-section" class="border-t border-gray-200 pt-6" style="display: {{ $user->role === 'organization' ? 'block' : 'none' }};">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Organization Information</h3>
                    
                    <div class="space-y-6">
                        @if($user->organization)
                            <div class="flex justify-between items-center">
                                <h4 class="text-sm font-medium text-gray-500">Organization: <span class="text-gray-900">{{ $user->organization->name }}</span></h4>
                                <a href="{{ route('admin.organizations.edit', $user->organization) }}" class="text-sm text-sky-600 hover:text-sky-800">Edit Organization</a>
                            </div>
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg">
                                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900">{{ $user->organization->email ?? 'N/A' }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-xs font-medium text-gray-500">Customer Service</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $user->organization->customer_service_number ?? 'N/A' }}</dd>
                                    </div>
                                    <div class="sm:col-span-2">
                                        <dt class="text-xs font-medium text-gray-500">Address</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $user->organization->address ?? 'N/A' }}</dd>
                                    </div>
                                </dl>
                            </div>
                        @else
